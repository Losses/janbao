...
CREATE TABLE `__new_bookmarks` (
	...
	`bookmarked_at` integer DEFAULT (strftime('%s', 'now') * 1000) NOT NULL,
...
CREATE TABLE `__new_conversation_reads` (
	`user_id` text NOT NULL,
	`conversation_id` text NOT NULL,
	`last_read_at` integer DEFAULT (strftime('%s', 'now') * 1000) NOT NULL,
...
CREATE TABLE `__new_users` (
	...
	`signup_time` integer DEFAULT (strftime('%s', 'now') * 1000) NOT NULL,
	`last_active_time` integer DEFAULT (strftime('%s', 'now') * 1000) NOT NULL,
