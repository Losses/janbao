...
1. For `/messages/[id]` ONLY, the new pipeline is the SOLE transition mechanism for every transition the route makes - the back-swipe gesture (slide to `/messages/inbox`, list-preview chip, Header morph, FAB scale, scroll-chrome) AND the non-gesture transitions (tab-click exit, deep-link landing). The behavior is indistinguishable from the old `GesturePageLayout`-driven behavior (verified by e2e).
...
7. E2E green: the existing specs that touch `/messages/<numeric>` (`tab-exit-preview`, `tab-click-transition`, `fab`) pass unchanged (behavior preservation), plus any new spec needed to cover the new pipeline's path.
...
- **Behavior preservation is the bar.** The pilot route's behavior must be as before. The existing e2e that touches the pilot route is the regression gate; if any existing e2e regresses, 5b1 is not done.
