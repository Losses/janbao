...
- **`FloatingActionButtonLayer` path precision** (Auditor 3 N4 / Auditor 5). On verification the DV12 docs never write the wrong `organisms/FloatingActionButtonLayer` path - they reference it path-agnostically (filename only) alongside the correct `organisms/Header.svelte` citations, so the auditor's "lives under organisms/" reading was a proximity misattribution; journal §5 note (e) already states `templates/`. The two path-agnostic plan mentions and the journal gate-result line were made explicit (`src/lib/components/templates/FloatingActionButtonLayer`) to remove the ambiguity. Cosmetic, zero behaviour.
