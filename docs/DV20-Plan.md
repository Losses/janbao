...
**Documentation.** The CMA writes `docs/DV20-Cycle-N-Journal.md` (the implementation journal: files changed, decisions, deviations from the spec, test results) and `docs/DV20-Meeting/DV20-Cycle-N-Audit-R{M}.md` per audit round. The journal records what actually happened, including failures; it does not perform confidence. Revision history of the Cycle spec lives in `docs/DV20-Meeting/DV20-Cycle-N-Plan-Journal.md`.
