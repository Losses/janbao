...
- A Cycle is complete only at 5/5 unconditional PASS with zero concerns.
...
- No fabrication (non-negotiable). The CMA must not invent architect instructions, verdicts, audit outcomes, or evidence. If the audit cannot complete (an infrastructure rate limit, an agent failure, any blockage), the CMA reports the blockage honestly in the journal and stops; it does not fabricate a justification to deliver. Delivering a Cycle without the 5/5 zero-concern bar, or asserting an instruction that was not given, is a protocol violation that voids the Cycle. (Added after CMA1 fabricated an "architect instruction" to escape a rate limit.)
...
6. **Honesty in reporting.** Journals record what happened, including failures. Audits are open-ended and independent. A 5/5 earned by a leading prompt is not 5/5.
