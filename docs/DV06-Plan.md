...
- **C01 - Installable shell.** `static/manifest.webmanifest`, `static/offline.html`,
  ...
  (sourced from IconKitchen, not generated), `app.html` meta, SW registration +
...
- **C03 - Offline-aware UX (disable sweep).** Expose the reactive `isOnline` flag
  via Svelte context; grey out every server-dependent affordance when offline
  (bookmark star, post/reply editors, settings/admin/messages/search entries); on
  forced-URL access to server-only pages, disable all forms; list pages show an
  "unavailable offline" empty state. Bookmark is disabled (not queued). The offline
  reader + banner remain the only active offline surface.
- **C04 - Lexical lazy-load + skeleton.** `LexicalEditorLazy.svelte` wrapper
  (same `LexicalEditorProps`, 1:1 zero-CLS skeleton); import swap in 3 write routes
  - 2 organism composers.
- **C05 - Web Push.** `pushSubscriptions` table + push prefs columns on
...
- `DV06-C01-Journal.md` … `DV06-C05-Journal.md` - per-cycle round logs.
- `RV06-C01-Audit-0<N>.md` … `RV06-C05-Audit-0<N>.md` - consolidated round reports.

## Deployment notes (ops checklist, not code defects)

- **Schema migrations:** C02 adds supporting indexes (no columns); C05 adds the
...
- **New deps:** `dexie` (runtime) for the offline IDB store. PWA icons are sourced
  from IconKitchen (committed binaries), not generated. No `web-push` (VAPID/aes128gcm
  via WebCrypto in C05).
