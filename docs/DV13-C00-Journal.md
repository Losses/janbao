...
- **(a) Mid-scrub `/search` -> deep over-freeze (auditor 1) - ADDRESSED as a defensive refinement.** The DV13 freeze `isSearch || searchScrubbing` could, on a `/search` -> deep navigation within the ~200ms scrub window, keep `iconProgress` at 0 (hamburger) on a deep page (where the arrow is correct) because Effect E returns at `curTabs === prevTabs` without cancelling the lingering scrub. Addressed by refining the freeze predicate to `isSearch || (searchScrubbing && currentHasTabs)` (`Header.svelte:195`): the scrub term is scoped to tab-root pages, so on a deep page (`currentHasTabs === false`) the icon tracks `1 - morph` (the arrow) even while the scrub's rAF is still driving `morph`. Empirically the over-freeze is practically unreachable through a real navigation: a `__headerMorphProbe` trace shows the deep-page load lands AFTER the scrub completes (`morph` is already 0 on the first sampled frame), so `searchScrubbing` is false at landing and the icon is correct regardless. A discriminator e2e cannot reliably catch the scrub in flight (deep nav latency >= scrub duration); the refinement stands on static correctness and is verified not to regress the DV13 flash fix by the suite (6/6, `maxTargetOnDest = 0.0deg` across two runs).
...
- **(d) Sampler wait hardened.** The fixed `waitForTimeout` after each back nav was replaced with `waitForIconSamplerDone` (waits for the rAF loop's `done` flag), eliminating a rare `destFrameCount` precondition flake; `destFrames` is now ~95 per case (was ~28-30).

Gates: `bun run check` 0/0 (1431 files); `bun test src/` 202 pass / 0 fail; `bun run test:e2e e2e/search-back-hamburger-flash.spec.ts` 6 pass / 0 fail over two runs; prettier + eslint + `tsc -p e2e/tsconfig.json` clean.
