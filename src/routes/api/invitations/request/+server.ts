...
export const POST: RequestHandler = async ({ locals, platform, url }) => {
	...
	try {
		if (isAdmin) {
			...
			// Count + insert inside one transaction so two concurrent requests
			// cannot both observe the old count and both exceed the monthly limit.
			...
			await db.transaction(async (tx: DbTransaction) => {
				const requestedThisMonth = await getMonthlyRequestCount(tx, user.id, window);
